...
#define MAKE_TOKEN(tableId, ofs)                                               \
  ((((int)(tableId)) << 24) | (((int)(ofs)) & 0x00ffffff))
#define GET_TABLE_ENTRY(type, pFile, token)                                    \
  (type *)(pFile->tables[TABLE_ID(token)].ppData[TABLE_OFS(token)])
...
tFile *LoadFile(char *pFilename) {
  ...
  if (VAL(char, pData, 0) != 'M' || VAL(char, pData, 1) != 'Z')
    Crash("Not an executable!");
  ...
  if (VAL(short, pPEHeader, 0) != 0x14c)
    Crash("This is not a .NET executable!");
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    memcpy(pRVA->pData, PTR(pData, rvaOfs),
           MIN(pRVA->size, VAL(int, pSection, 16)));
  ...
  printf("Runtime version: %i.%i\n", VAL(short, pCLIHeader, 4),
         VAL(short, pCLIHeader, 6));
    ...
    if (strcmp(pStreamName, "#~") == 0) {
      ...
      for (int i = 0; i < 64; i++) {
        ...
        if (count != 0) {
          ...
          void **ppData = pFile->tables[i].ppData =
              malloc(sizeof(void *) * (count + 1));
          ppData[0] = NULL;
...
tMethodState *CreateMethodState(tMD_MethodDef *pMethodDef) {
  ...
  if ((VAL(char, pILHeader, 0) & 0x3) != 0x2)
    Crash("Can only understand Tiny IL Headers");
...
void Execute(tMethodState *pMethodState) {
  ...
  for (;;) {
    ...
    case 0x28: // CALL
    {
      ...
      // Only method-defs are handle currently. I.e. the method being called
      // must in this assembly.
      int callToken = VAL(int, pIL, pMethodState->ip);
      pMethodState->ip += 4;
      // Load the method-definition from the metadata, and create a method
      // state.
      ...
      // If it has a return-value, copy it to to evaluation-stack of this
      // method.
      memcpy(PTR(pEvalStack, pMethodState->esp), pCallMethodState->evalStack,
             pCallMethodState->esp);
    ...
    case 0x58: // ADD
    {
      ...
      // Read the top two int32 values from the evaluation stack, add them, then
      // push back on to the evaluation stack.
...
int main(int argc, char **argp) {
  ...
  tMD_MethodDef *pEntryPointMethodDef =
      GetMethodByToken(pFile, pFile->entryPointToken);
  ...
  if (pMethodState->esp == 4) {
    // If an int32 value is left at position 0 in the evaluation stack, then
